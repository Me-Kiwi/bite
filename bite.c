...
void hande_CSI(editorState* State, escseq key, row_state * r_state) {
  switch(key) {
    case UP:
        ...
        State->fileposition_x = min(r_state[State->index_table[State->fileposition_y]].no_of_char - 1, State->fileposition_x);
      break;
    case DOWN:
      ...
      break;
    case RIGHT:
      if(State->fileposition_x == r_state[State->index_table[State->fileposition_y]].no_of_char - 1){
    ...
    case LEFT:
      if(State->fileposition_x == 0) {              
         if(State->fileposition_y - 1 >= 0 ){  
           State->fileposition_y--;
           State->fileposition_x = r_state[State->index_table[State->fileposition_y]].no_of_char - 1;
        }
      } else {  
        State->fileposition_x--;
      }
...
void compile_and_execute(editorState *State, row_state *r_state) {
    save_to_file(State, r_state);

    system("gcc -o /home/kiwi/bite/code.c /home/kiwi/bite/code.out");

    system("/home/kiwi/bite/code.out");
}
...
void normal_mode(editorState *State, row_state *r_state, char input) {
    switch(input) {
        ...
        case 'l':
            ...
            break;
        case 'r':
            compile_and_execute(State, r_state);
            break;
        case 'd':
